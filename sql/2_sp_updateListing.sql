...
BEGIN
	...
	--         (SELECT TRY_CAST(value AS int) AS productClassificationId
	...
	BEGIN
		...
		BEGIN
			...
			ELSE IF NOT EXISTS (SELECT 1 FROM productClassification
			                    WHERE ACTIVE = 1
								AND productClassificationId = (SELECT TRY_CAST(productClassificationId AS int)
	...
	ELSE IF (@inp_userId = (SELECT userId from listings WHERE listingId = @inp_listingId AND ACTIVE = 1)
			 ...
			 AND ( (SELECT CHECKSUM_AGG(CHECKSUM(CAST(inp.value AS int))) FROM STRING_SPLIT(@inp_productClassificationIdList, ';') inp)
	...
	BEGIN TRY;
		...
		CREATE TABLE #temp_listings_productClassification_old (productClassificationId int);
		...
		--AND productClassificationId IN (SELECT CAST(value AS int) FROM STRING_SPLIT(@inp_attachmentUrlList,';'))
		...
		SELECT @inp_listingId, CAST(value AS int) FROM STRING_SPLIT(@inp_productClassificationIdList,';');
