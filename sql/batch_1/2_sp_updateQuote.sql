...
CREATE PROCEDURE sp_updateQuote
   ...
   @inp_quoteStatusId bigint,
   @inp_quoteCurrencyId bigint,
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			IF NOT EXISTS (SELECT 1 FROM deliveryMethod
			               WHERE ACTIVE = 1
						   AND deliveryMethodId = (SELECT TRY_CAST(deliveryMethodId AS bigint)
	...
	ELSE IF (@inp_userId = (SELECT userId from quotes WHERE quoteId = @inp_quoteId AND ACTIVE = 1)
			 ...
			 AND ( (SELECT CHECKSUM_AGG(CHECKSUM(CAST(dm.value AS bigint), CAST(da.value AS decimal(10,2))))
	...
	BEGIN TRY;
		...
		CREATE TABLE #temp_quotes_deliveryMethod_old (deliveryMethodId bigint);
		...
		SELECT @inp_quoteId, CAST(dm.value AS bigint), CAST(da.value AS decimal(10,2))
		FROM STRING_SPLIT(@inp_deliveryMethodIdList,';',1) dm
		JOIN STRING_SPLIT(@inp_deliveryAmountList,';',1) da ON dm.ordinal = da.ordinal;
