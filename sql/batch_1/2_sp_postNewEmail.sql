...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			ELSE IF NOT EXISTS (SELECT 1 FROM users
			                    WHERE ACTIVE = 1
								AND id = (SELECT CAST(emailRecipient AS bigint)
	...
	BEGIN TRY;
		...
		BEGIN
			...
			SELECT @out_emailId, CAST(emailRecipient AS bigint), @out_runId
		...
		WHILE (@attachmentIterator <= (SELECT MAX(rowNum) FROM #temp_emailAttachments))
		BEGIN
			DECLARE @out_attachmentId bigint;
