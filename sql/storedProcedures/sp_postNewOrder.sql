...
CREATE PROCEDURE sp_postNewOrder
   @inp_listingId int,
   @inp_quote_deliveryMethodId int,
...
BEGIN
	...
	BEGIN
		SET @ERR_MESSAGE = 'Invalid input(s). listingId, quote_deliveryMethodId, orderStatusId and overrideQuote must not be null.';
		SET @ERR_IND = 1;
	END
	...
	ELSE IF (@inp_quote_deliveryMethodId IS NOT NULL
	         AND (SELECT COUNT(*) FROM quotes_deliveryMethod WHERE quote_deliveryMethodId = @inp_quote_deliveryMethodId AND ACTIVE = 1) = 0)
	BEGIN
	...
	BEGIN TRY;
		...
		VALUES ('sp_postNewOrder');
		SET @out_runId = (SELECT MAX(runId) from runIds WHERE processName = 'sp_postNewOrder');
